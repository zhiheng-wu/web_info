...
**data：**     用户评分数据、用户评分矩阵、用户相似度矩阵、测试集及数据集等（用于实验一第二阶段前半部分）

**data2-2：**  各物品向量化后的数据、物品相似度矩阵、测试集及数据集（用于实验一第二阶段后半部分）

下载data后，运行src/rec中各sort程序进行协同过滤排序时，需要更改路径（sort predict data_process等）
